...
var LazyRipple = class _LazyRipple {
  ...
  static use() {
    ...
    const [shouldMount, setShouldMount] = React40.useState(false);
    ripple.shouldMount = shouldMount;
    ripple.setShouldMount = setShouldMount;
    React40.useEffect(ripple.mountEffect, [shouldMount]);
    return ripple;
  }
  mount() {
    if (!this.mounted) {
...
function Ripple(props) {
  ...
  const [leaving, setLeaving] = React47.useState(false);
...
`;
var TouchRipple = React48.forwardRef(function TouchRipple2(inProps, ref) {
  ...
  const [ripples, setRipples] = React48.useState([]);
  const nextKey = React48.useRef(0);
  const rippleCallback = React48.useRef(null);
  React48.useEffect(() => {
  ...
  const ignoringMouseDown = React48.useRef(false);
  const startTimer = useTimeout();
  const startTimerCommit = React48.useRef(null);
  const container = React48.useRef(null);
  const startCommit = React48.useCallback((params) => {
    const {
      pulsate: pulsate2,
      rippleX,
      rippleY,
  ...
  const start = React48.useCallback((event = {}, options = {}, cb = () => {
  }) => {
    const {
  ...
  const pulsate = React48.useCallback(() => {
    start({}, {
      pulsate: true
    });
  }, [start]);
  const stop = React48.useCallback((event, cb) => {
    startTimer.clear();
  ...
  React48.useImperativeHandle(ref, () => ({
    pulsate,
    start,
    stop
  }), [pulsate, start, stop]);
...
var ButtonBase = React49.forwardRef(function ButtonBase2(inProps, ref) {
  const props = useDefaultProps2({
    props: inProps,
    name: "MuiButtonBase"
  });
